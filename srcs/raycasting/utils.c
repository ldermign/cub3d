...
/*   Updated: 2022/04/09 19:29:51 by ejahan           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_background(t_cub *cub, int x, int y)
{
	...
	while (a < x)
	{
		...
		while (b < y / 2)
		{
			dst = cub->addr + (b * cub->line_length + a * (cub->bpp / 8));
			*(unsigned int *)dst = cub->c;
			b++;
		}
		while (b < y)
		{
			dst = cub->addr + (b * cub->line_length + a * (cub->bpp / 8));
			*(unsigned int *)dst = cub->f;
			b++;
		}
		a++;
	}
...
void	window(t_struct *all)
{
	...
	all->cub.mlx_ptr
		= mlx_new_window(all->cub.mlx, all->cub.x, all->cub.y, "Cub3D");
	all->cub.img = mlx_new_image(all->cub.mlx, all->cub.x, all->cub.y);
	all->cub.addr = mlx_get_data_addr(all->cub.img, &all->cub.bpp,
