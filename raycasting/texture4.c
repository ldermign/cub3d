...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/06/03 17:47:23 by ejahan            #+#    #+#             */
/*   Updated: 2021/06/06 15:11:58 by ejahan           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	raycast1(t_cub *cub)
{
	...
	cub->texHeight = 64;
	cub->texWidth = 64;
	texture = malloc((sizeof(char) * (cub->texWidth * cub->texHeight)));
	...
	while (x < cub->texWidth && y < cub->texHeight)
	{
		...
		texture[cub->texWidth * y + x] = (char)(65536 * 254 * (x != y && x != cub->texWidth - y));
		x++;
		y++;
	}
	...
	while (x < cub->x)
	{
		...
		while (y < cub->drawEnd)
		{
			...
			cub->texPos += cub->step;
			color = (unsigned int)(texture[cub->texHeight * cub->texY + cub->texX] * 2);
			// if (cub->side == 1)
			// 	color = 321654;
			// buffer[y][x] = color;
