...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/06/10 16:07:09 by ejahan            #+#    #+#             */
/*   Updated: 2021/06/22 21:53:24 by ejahan           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	recup_textures(t_cub *cub)
{
	...
	cub->texture_no = mlx_xpm_file_to_image(cub->mlx, cub->north,
				&(cub->width_no), &(cub->height_no));
	cub->addr_no = mlx_get_data_addr(cub->texture_no, &i, &i, &i);
	// cub->texture_ea = mlx_xpm_file_to_image(cub->mlx, cub->east,
	// 			&(cub->width_ea), &(cub->height_ea));
	// cub->addr_ea = mlx_get_data_addr(cub->texture_ea, &i, &i, &i);
	// cub->texture_we = mlx_xpm_file_to_image(cub->mlx, cub->west,
	// 			&(cub->width_we), &(cub->height_we));
	// cub->addr_ea = mlx_get_data_addr(cub->texture_we, &i, &i, &i);
	// cub->texture_so = mlx_xpm_file_to_image(cub->mlx, cub->south,
	// 			&(cub->width_so), &(cub->height_so));
	// cub->addr_ea = mlx_get_data_addr(cub->texture_so, &i, &i, &i);
...
void	raycast1(t_cub *cub)
{
	...
	while (x < cub->x)
	{
		...
		// 	cub->texPos += cub->step;
		// 	color = (int)cub->addr_no[cub->texHeight * cub->texY + cub->texX];
		// 	if (cub->side == 1)
		// 		color /= 2;
		...
		if (cub->side == 1)
			color /= 2;
		// else if (cub->side == 1 && (cub->dirX) >= 0)
		// 	color = create_trgb(21, 0, 0, 0);
		// else if (cub->side == 0 && cub->raydirX < 0)
		// 	color = create_trgb(21, 120, 250, 120);
